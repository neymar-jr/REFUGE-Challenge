...
# x = torch.randn(size = (4,1,20,20)).to(device)
# Danet = DANet(1).to(device)
# y = Danet(x)
# print('y.size:',y.size())     
# print(y.max())  
...
class CC_module(nn.Module):
    ...
    def forward(self, x):
        ...
        # energy_H = (energy_H + self.INF(m_batchsize, height, width).to(x.device)).view(m_batchsize,width,height,height).permute(0,2,1,3)
